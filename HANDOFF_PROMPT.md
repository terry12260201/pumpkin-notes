...
**2026-09-20 晚**：雲端筆記改成「fetch 後 document.write 原地顯示」（不再用 blob: 網址，blob 頁面的登入狀態／收藏在部分瀏覽器失效，南瓜實測按收藏無反應）。
...
**待辦（照順序）**
1. ✅ **正式網址 https://notes.pumpkinvrarai.com/（2026-09-20 完成）**：網域 pumpkinvrarai.com 南瓜自己在 Cloudflare 買的（zone 567f9608…，DNS token 在 `~/.config/pumpkin-notes/config.json` 的 `cloudflare_token`，只能改這個網域的 DNS）；CNAME notes → terry12260201.github.io（DNS only）；GitHub Pages 已綁＋強制 HTTPS；github.io 舊網址自動轉過來。config.js／worker／README 已改。Supabase Site URL 也要是新網址。舊記錄：：南瓜指示不用 Wix 管的 pumpkinvr.com。改用公司信箱網域 **pumpkinvrar.com**（DNS 在 Cloudflare、登記 GoDaddy），子網域定 `notes.pumpkinvrar.com`：Cloudflare 加 CNAME `notes` → `terry12260201.github.io`，**Proxy 關閉（DNS only，灰雲）**；誰有 Cloudflare 帳號待南瓜確認（或給一把只能改該區 DNS 的 API token 由 AI 加）。生效後才綁 GitHub Pages、改 config.js／worker／Supabase URL。原記錄：：⚠️ 綁上自訂網域後 GitHub 會把 github.io 全部轉向新網域，DNS 沒生效＝整站死。已拆掉綁定、程式退回 github.io。**正確順序：先等 `dig pn.pumpkinvr.com` 查得到 CNAME，再綁。** 南瓜已把 Supabase Site URL 改成 pn（要先改回 github.io，等網域通再改）。原記錄：：定案 `pn.pumpkinvr.com`。已做：GitHub Pages 綁定（repo 有 CNAME 檔）、`data/config.js siteUrl`、`worker.py SITE`、README 全改新網址。**卡在 Wix 的 CNAME 還查不到**（ns0.wixdns.net 直接問也沒有），等南瓜確認記錄；生效後 GitHub 會自動發 HTTPS 憑證（最多 1 小時），再開 `https_enforced`，並請南瓜到 Supabase → Authentication → URL Configuration 把 Site URL 改 `https://pn.pumpkinvr.com/`、Redirect URLs 加同一條。原文：DNS 在 Wix（`ns0/ns1.wixdns.net`）。`notes.pumpkinvr.com` **已有一筆 CNAME 指到 `cname.vercel-dns-0.com`**（目前打不開，疑似舊設定），要南瓜決定：改這筆指到 `terry12260201.github.io`，或另用 `pn.pumpkinvr.com`（目前空著）。DNS 好了之後：`gh api -X PUT repos/terry12260201/pumpkin-notes/pages -f cname=<網域>` → 等 HTTPS 憑證 → 改 `data/config.js siteUrl`、`worker.py SITE`、Supabase Authentication → URL Configuration 的 Site URL 與 Redirect URLs、Google OAuth 同意畫面的授權網域。
2. ✅ **Google 雲端存放（2026-09-20 完成並實測）**：worker 上架後 `externalize_images`（截圖抽出→1280 寬 JPEG q80→公開桶 `img/<uid>/<slug>/NN.jpg`，網頁從 2MB→80KB）→ `drive_store.archive_note`（南瓜 OAuth 授權，`google_oauth` 在 config.json；Drive API 已啟用、用戶端 pumpkin-notes-worker）→ 刪本機工作夾。規則見 `worker/STORAGE_RULES.md`。既有 13 篇的圖已歸檔到雲端硬碟。舊記錄：：南瓜雲端 `Pumpkin Notes 素材庫/`（https://drive.google.com/drive/folders/1P_qA5brzC137GlEQm37tGlnxCgBl6fLL ）底下 01 報告網頁／02 封面圖／03 影片截圖／04 夥伴上傳；資料夾 id 在 `~/.config/pumpkin-notes/config.json` 的 `drive_folders`。worker 尚未接上傳（需服務帳戶金鑰 `drive_sa.json`，並把素材庫分享給服務帳戶信箱）。原規劃：：在公司 Google Drive 建共用資料夾 `Pumpkin Notes 素材庫/`，底下 `報告HTML/`、`封面/`、`截圖幀/<slug>/`、`夥伴上傳/`；worker 上架後多一步用 Drive API 上傳（HTML、封面 jpg、幀 jpg）。憑證走 **服務帳戶**：GCP 專案 `pumpkin-meeting-platform` → IAM → 服務帳戶 → 建立 → 金鑰 JSON 存 `~/.config/pumpkin-notes/drive_sa.json`；把資料夾分享給服務帳戶信箱（編輯者）即可，不用 domain-wide delegation。夥伴自己拍的照片直接丟 `夥伴上傳/`，worker 之後可掃該夾當錄影檔來源。
...
**產品名維持 Pumpkin Notes（2026-09-21 南瓜確認不改名）**，一句話：把影片，變成團隊用得上的筆記。
