@echo off
chcp 65001 >nul
REM 南瓜數位筆記整理主機（PC-01 常駐）。當掉就自動重開；連續重開會發 Telegram 告訴南瓜。
...
if %CRASHES% GEQ 3 (
  ...
  if not "%TGT%"=="" curl -s -o nul -X POST "https://api.telegram.org/bot%TGT%/sendMessage" --data-urlencode "chat_id=%TGC%" --data-urlencode "text=🎃 南瓜數位筆記整理主機（PC-01）%0A🔴 worker 連續 %CRASHES% 次啟動失敗，請看 worker.log（%~dp0..\worker.log）" 
  set CRASHES=0
